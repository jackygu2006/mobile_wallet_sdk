const polkadotTypes = (function (exports, util, utilCrypto) {
  ...
  const definitions$c = {
    rpc: {},
    types: _objectSpread$n(_objectSpread$n(_objectSpread$n(_objectSpread$n(_objectSpread$n({}, junction), multiAsset), xcm), xmcOrder), {}, {
      DoubleEncodedCall: {
        encoded: 'Vec<u8>'
      },
      XcmOriginKind: {
  ...
  */
      ...
      return class extends CodecSet {
        constructor(registry, value) {
          ...
          Object.keys(values).forEach(_key => {
            const name = util.stringUpperFirst(util.stringCamelCase(_key));
            const iskey = `is${name}`;
            util.isUndefined(this[iskey]) && Object.defineProperty(this, iskey, {
              enumerable: true,
  ...
  (function (TypeDefInfo) {
    ...
    TypeDefInfo[TypeDefInfo["Si"] = 17] = "Si";
    TypeDefInfo[TypeDefInfo["Null"] = 18] = "Null";
  ...
  function checkInstance(created, value) {
    const u8a = created.toU8a();
    const rawType = created.toRawType();
    util.assert(util.u8aEq(value, u8a) || // when length-prefixed from hex, just check the actual length
    ['Bytes', 'Text', 'Type'].includes(rawType) && value.length === created.length, () => `${rawType}:: Decoded input doesn't match input, received ${util.u8aToHex(value, 512)} (${value.length} bytes), created ${util.u8aToHex(u8a, 512)} (${u8a.length} bytes)`);
  }
  ...
  function _extract2(type, lookupIndex) {
    ...
    try {
      ...
      throw new Error(`PortableRegistry: ${lookupIndex}: Error extracting ${util.stringify(type)}: ${error.message}`);
    }
  ...
  function _extractArray2(lookupIndex, {
    ...
    util.assert(!length || length.toNumber() <= 256, () => `PortableRegistry: ${lookupIndex}: Only support for [Type; <length>], where length <= 256`);
  ...
  function _extractCompositeSet2(lookupIndex, params, fields) {
    util.assert(params.length === 1 && fields.length === 1, () => `PortableRegistry: ${lookupIndex}: Set handling expects since param and single field`);
    return withTypeString(this.registry, {
  ...
  function _extractFields2(lookupIndex, fields) {
    ...
    util.assert(isTuple || isStruct, () => `PortableRegistry: ${lookupIndex}: Invalid fields type detected, expected either Tuple (all unnamed) or Struct (all named)`);
  ...
  function _extractVariant2(lookupIndex, {
    ...
    if (specialVariant === 'Option') {
      ...
      return withTypeString(this.registry, {
        ...
        sub: params.map(({
          type
        }) => _classPrivateFieldBase(this, _createSiDef)[_createSiDef](type.unwrap())).map((def, index) => _objectSpread$7({
          name: ['Ok', 'Error'][index]
        }, def))
      });
  ...
  function getModuleTypes({
    ...
    return _objectSpread$4(_objectSpread$4({}, typesModules[section] || {}), ((_knownTypes$typesAlia = knownTypes.typesAlias) === null || _knownTypes$typesAlia === void 0 ? void 0 : _knownTypes$typesAlia[section]) || {});
  }
  ...
  function extendPrefixedMap(registry, itemFn, storageFn) {
    ...
    storageFn.iterKey = extendHeadMeta(registry, itemFn, storageFn, (...args) => {
      util.assert(args.length === 0 || type.isDoubleMap && args.length === 1 || type.isNMap && args.length === type.asNMap.hashers.length - 1, () => `Iteration ${util.stringCamelCase(section || 'unknown')}.${util.stringCamelCase(method || 'unknown')} needs arguments to be one less than the full arguments, found [${args.join(', ')}]`);
