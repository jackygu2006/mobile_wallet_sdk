...
function checkInstance(created, value) {
  ...
  (0, _util.assert)((0, _util.u8aEq)(value, u8a) || // when length-prefixed from hex, just check the actual length
  ['Bytes', 'Text', 'Type'].includes(rawType) && value.length === created.length, () => `${rawType}:: Decoded input doesn't match input, received ${(0, _util.u8aToHex)(value, 512)} (${value.length} bytes), created ${(0, _util.u8aToHex)(u8a, 512)} (${u8a.length} bytes)`);
}
